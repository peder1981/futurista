...
class EconomicVisualizer:
    ...
    def create_comparative_bar_chart(df, year, top_n=10, indicator="ratio"):
        ...
        # Criar figura com suporte para eixo secundário
        fig = make_subplots(specs=[[{"secondary_y": True}]])
        
        # Configurar título
        fig.update_layout(
            title_text=f"📊 Comparativo: Juros vs Inflação — Top {top_n} ({year})"
        )
        
        # Adicionar barras de juros
        fig.add_trace(
            go.Bar(
        ...
        if indicator == "ratio":
            ...
            fig.update_yaxes(
                title_text="Razão Juros/Inflação",
                secondary_y=True
            )
